...
public class varifyUtils {
    ...
    public void search(String rootpath,String parentroot) throws IOException {
        ...
        if(!root.isDirectory()) {
            map.put(root.getName(),new varifyUtils().getFileMD5String(root));
            return;
        }
...
//                    System.out.println((rootpath + "/" + aSrcList.getName()).substring(parentroot.length()+1));
                    map.put((rootpath + "/" + aSrcList.getName()).substring(parentroot.length()+1), new varifyUtils().getFileMD5String(new FileInputStream(current)));
                }
            }
        }
    }
